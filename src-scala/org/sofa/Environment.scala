...
/**
 * Representation of a set of parameters that can be read from the command line or configuration
 * files and automatically stored in objects.
 * 
 * <p>
	...
	def readConfigFile( fileName:String ) {
		try {
			Source.fromFile( new File( fileName ) ).getLines().foreach {
				case fileEqualParamRE(key,value) => {
					parameters.put( key, value )
				}
				case fileCommentRE( value ) => { /* ignore */ }
				case fileEmptyRE( value ) => { /* ignore */ }
				case value:String => {
					printf( "Environment: line %s cannot be parsed%n", value )
				}
			}
		} catch {
