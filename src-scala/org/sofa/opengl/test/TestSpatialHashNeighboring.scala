...
class TestParticle2(xx:Double, yy:Double, zz:Double) extends SpatialPoint {
...
class TestVolume2(val side:Double) extends TestParticle2(-side/2,-side/2, -side/2) with SpatialCube {
...
class TestSpatialHashNeighboring extends SurfaceRenderer {
	...
	var spaceHash = new SpatialHash[TestParticle2](bucketSize)
	var simu:ArrayBuffer[TestParticle2] = null
	val simuCube = new TestVolume2(0.4)
	...
	protected def initParticles(n:Int) {
		...
		var p = new TestParticle2(0, 1, 0); simu += p; spaceHash.add(p); particlesMesh.setPoint(0, p.x); particlesMesh.setColor(0, Rgba.red)
		val angle = (math.Pi*2) / (n-1)
		var a = 0.0
		
		for(i <- 1 until n) {
			p = new TestParticle2(math.cos(a), 1, math.sin(a))
			a += angle
			simu += p
			spaceHash.add(p)
...
//		plane.draw(planeMesh.drawAs)
	...
	protected def updateParticles() {
		val potential = spaceHash.neighborsInBox(simu(0), 4)
		
		Console.err.println("neighbors %d (%d particles + 1 cube)".format(potential.size, simu.size))
