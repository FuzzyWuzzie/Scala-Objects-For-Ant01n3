...
//	SurfaceExecutorService.configure(50)
	...
	def start() {
        ...
        renderer.ctrl           = new TestRobot2Interaction(renderer.camera, renderer)
...
class TestRobot2 extends SurfaceRenderer {
	import TestRobot2._
...
// Rendering
	...
	protected def initBehaviors() {
		robot = new RobotZazou3(armature)
		robot.start
	}
...
class RobotZazou3(val armature:Armature) {
...
class TestRobot2Interaction(camera:Camera, val renderer:TestRobot2) extends BasicCameraController(camera) {
