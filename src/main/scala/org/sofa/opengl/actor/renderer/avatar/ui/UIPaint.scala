...
import org.sofa.opengl.{ ShaderResource, Space }
import org.sofa.opengl.actor.renderer.{ Screen }
import org.sofa.opengl.actor.renderer.{ Avatar, DefaultAvatar, DefaultAvatarComposed, DefaultAvatarMixed, AvatarName, AvatarRender, AvatarInteraction, AvatarSpace, AvatarContainer, AvatarFactory, DefaultAvatarFactory, AvatarSpaceState, AvatarState, AvatarBaseStates }
...
abstract class UIPaint(name: AvatarName, screen: Screen) extends DefaultAvatarMixed(name, screen) {
	...
	def consumeEvent(event: org.sofa.opengl.actor.renderer.AvatarEvent): Boolean = {
		false
	}
