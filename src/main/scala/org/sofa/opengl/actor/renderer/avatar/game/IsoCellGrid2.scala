...
//   *
//   * @param config is a 2D array of [[IsoCellConfig]].
//   * @param groundTexture is the texture to use to render the color of the cells surface.
//   * @param groundMask is the texture to use to light the cells surface.
//   * @param undergroundTexture is the texture to use to render the color of the cells terrain.
//   * @param undergroundMask is the texture to use to light the cells terrain.
//   * @param shader is the shader to use to render the cells. */
// case class IsoCellGridConfig(config:Array[Array[IsoCellConfig]],
// 					groundTexture:String, groundMask:String,
// 					undergroundTexture:String, undergroundMask:String,
// 					shader:String) extends AvatarRenderState {}
...
// 	 	state match {
// 	 		case IsoCellGridConfig(config, gtex, gmsk, ugtex, ugmsk, shd) => if(config.length >= Height && config(0).length >= Width) {
// 	 			init(config, gtex, gmsk, ugtex, ugmsk, shd)
// 	 		} else {
...
// 	protected def init(config:Array[Array[IsoCellConfig]], gtex:String, gmsk:String, ugtex:String, ugmsk:String, shd:String) {
...
// 		isoShader = screen.libraries.shaders.get(gl, shd)//, ShaderResource("iso-shader", "iso.vert.glsl", "iso.frag.glsl"))
		
// 		groundColor      = screen.libraries.textures.get(gl, gtex)//,  TextureResource("ground-color-1",      "IsoTemplate_1024_Ground.png",           TexParams()))
// 		undergroundColor = screen.libraries.textures.get(gl, ugtex)//, TextureResource("underground-color-1", "IsoTemplate_1024_Underground.png",      TexParams()))
// 		groundMask       = screen.libraries.textures.get(gl, gmsk)//,  TextureResource("ground-mask-1",       "IsoTemplate_1024_Ground-Mask.png",      TexParams()))
// 		undergroundMask  = screen.libraries.textures.get(gl, ugmsk)//, TextureResource("underground-mask-1",  "IsoTemplate_1024_Underground-Mask.png", TexParams()))
