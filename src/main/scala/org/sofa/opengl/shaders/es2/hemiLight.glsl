/** Compute the lighted color of position P with normal N, according to
  ...
  * of the hemilight. */
vec4 hemiLightWhithSky(HemisphereLight hemilight, vec3 P, vec3 N) {
...
vec4 hemiLight(HemisphereLight hemilight, vec3 P, vec3 N, vec4 color) {
   vec3  L = normalize(hemilight.pos - P);
   float A = dot(N, L) * 0.5 + 0.5;
...
/** Compute the lighted color of position P with normal N, according to
  ...
  * color. */
vec4 hemiLight(HemisphereLight hemilight, vec3 P, vec3 N, vec4 color, vec4 shadow) {
   vec3  L = normalize(hemilight.pos - P);
   float A = dot(N, L) * 0.5 + 0.5;
