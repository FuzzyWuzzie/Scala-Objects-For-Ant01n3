...
// Rendering
	...
	protected def initTextures() {
	    ...
	    thingColor = new Texture(gl, "textures/Armature_Color_002.png", TexParams(mipMap=TexMipMap.Generate))
	    thingColor.minMagFilter(gl.LINEAR_MIPMAP_LINEAR, gl.LINEAR)
	    thingColor.wrap(gl.REPEAT)

	    thingNMap = new Texture(gl, "textures/Armature_NMap_002.png", TexParams(mipMap=TexMipMap.Generate))
