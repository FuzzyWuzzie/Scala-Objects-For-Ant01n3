...
import org.sofa.gfx.{SGL, Camera, VertexArray, ShaderProgram, Texture, Shader, HemisphereLight, TexParams, TexMin, TexMag, TexMipMap, TexAlpha, Libraries, ShaderResource, TextureResource, ArmatureResource}
...
import org.sofa.gfx.armature.behavior.{ArmatureBehavior, ArmatureBehaviorLoader, LerpKeyArmature, Switch}
import org.sofa.gfx.surface.{Surface, SurfaceRenderer, BasicCameraController}
import org.sofa.gfx.surface.event._
import org.sofa.gfx.mesh.{PlaneMesh, Mesh, BoneMesh, EditableMesh, VertexAttribute, LinesMesh}
...
//	SurfaceExecutorService.configure(50)
	...
	def start() {
	    ...
	    renderer.actionKey      = renderer.ctrl.actionKey
	    renderer.motion         = renderer.ctrl.motion
	    renderer.gesture        = renderer.ctrl.gesture
	    renderer.surface        = new org.sofa.gfx.backend.SurfaceNewt(renderer,
...
// Rendering
	...
	protected def initArmatures(armatureFileName:String) {
		libraries.armatures += ArmatureResource("armature-test", "armature-texture", "armature-shader", armatureFileName, "Armature", 0.001, libraries, null)
	}
...
class ArmatureKeyAnimatorInteraction(camera:Camera, val renderer:ArmatureKeyAnimator) extends BasicCameraController(camera) {
	...
	override def actionKey(surface:Surface, e:ActionKeyEvent) {
		println("TODO ArmatureKeyAnimatorInteraction.actionKey")
...
println("TODO ArmatureKeyAnimatorInteraction.gesture")
//		zoom(e.amount * step * 0.05)
	} 

	override def motion(surface:Surface, event:MotionEvent) {
println("TODO ArmatureKeyAnimatorInteraction.motion")
		// if(event.isStart) {
