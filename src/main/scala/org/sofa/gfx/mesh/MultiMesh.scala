...
trait MultiMesh extends Mesh {
    ...
    def lastVertexArray(subMesh:Int):VertexArray = vas(subMesh)

    /** The last created vertex array for `subMesh`. Synonym of `lastVertexArray`. */
    def lastva(subMesh:Int):VertexArray = vas(subMesh)
    ...
    def newSubMeshVertexArray(gl:SGL, subMesh:Int, drawMode:Int, shader:ShaderProgram, locations:Tuple2[String,String]*):VertexArray = {
    	...
    	newSubMeshVertexArray(gl, subMesh, drawMode, locs:_*)
    }
    ...
    /** Create a vertex array from the given map of attribute names / locations.
      ...
      * The last created vertex array is remembered by the mesh and can be accessed later,
      * and for some meshes updated from new data if the mesh is dynamic. */
    def newSubMeshVertexArray(gl:SGL, subMesh:Int, drawMode:Int, locations:Tuple2[String,Int]*):VertexArray = {
