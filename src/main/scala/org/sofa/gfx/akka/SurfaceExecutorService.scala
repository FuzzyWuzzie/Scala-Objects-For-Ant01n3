...
object SurfaceExecutorService extends AbstractExecutorService { 
	...
	final val configKey = "org.sofa.gfx.akka.surface-dispatcher"
	...
	def configure(tickDurationMs:Long) {
		...
		sys.props += (s"${configKey}.executor"       → "org.sofa.gfx.akka.SurfaceExecutorServiceConfigurator")
	...
	def config(tickDurationMs:Long):Config = {
		ConfigFactory.parseMap(HashMap[String,AnyRef](
				...
				(s"${configKey}.executor"       → "org.sofa.gfx.akka.SurfaceExecutorServiceConfigurator"),
...
/** Create an ExecutorServiceConfigurator so that Akka can use our
  ...
  *    surface-dispatcher {
  *        type       = "Dispatcher"
  *        executor   = "org.sofa.gfx.akka.SurfaceExecutorServiceConfigurator"
  *        throughput = 1
  *    }
