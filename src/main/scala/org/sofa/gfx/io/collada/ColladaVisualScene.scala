...
import org.sofa.gfx.mesh.skeleton.Bone
import scala.math._
...
class VisualScene(node:Node) extends ColladaFeature {
	...
	/** Try to swap axis considering the source uses Blender axis and pass them to OpenGL axis.
	  ...
	  * when the mesh is transformed to a SOFA [[org.sofa.gfx.mesh.Mesh]] when calling toMesh()`. */
	def blenderToOpenGL(on:Boolean) { blenderToOpenGLCoos = on }

	/** Convert the given node and sub-nodes to a SOFA [[org.sofa.gfx.mesh.skeleton.Bone]] hierarchy.
