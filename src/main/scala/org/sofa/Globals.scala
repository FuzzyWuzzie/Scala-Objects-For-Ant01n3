...
Nulla nec felis sed leo placerat imperdiet. Aenean suscipit nulla in justo.
...
Curabitur eu amet.""".replace(",", "").replace("\n", " ").replace(".", "").trim

	val random = new Random()

	def randomWord(averageLength:Int=10):String = {
		val len = if(averageLength >= 2) averageLength else 2
		val beg = LoremIpsum.indexOf(' ', random.nextInt(LoremIpsum.length - averageLength*3)) + 1
		var end = LoremIpsum.indexOf(' ', beg)

		if(end < 0) end = LoremIpsum.length

		LoremIpsum.substring(beg, end)
	}
	
	def randomString(length:Int=10):String = {
		val beg = random.nextInt(LoremIpsum.length - length) 
		val end = beg + 20
		LoremIpsum.substring(beg, end)
	}
